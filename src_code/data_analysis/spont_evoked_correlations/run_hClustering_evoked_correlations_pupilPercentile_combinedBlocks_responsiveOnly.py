...
for session in all_sessions_to_run:
    ...
    fname = ('evoked_correlations_pupilPercentile_combinedBlocks_%s_windLength%0.3fs_%s.mat' % (session, wind_length, data_name))
    ...
    savemat(('%s%s_responsiveOnly_windLength%0.3fs_rateThresh%0.3fHz_hClustering_%s%s.mat' % (outpath, session, wind_length, rate_thresh, cells_toKeep,data_name)), data_save)
