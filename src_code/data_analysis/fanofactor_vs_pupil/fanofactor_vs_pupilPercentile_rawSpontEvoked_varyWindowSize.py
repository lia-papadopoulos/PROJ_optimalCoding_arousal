...
window_length = settings.window_length_sweep
...
#%% store number of trials we can subsample at each window size
nTrials_subsample_evoked = np.zeros(len(window_length))
nTrials_subsample_spont = np.zeros(len(window_length))
...
trial_window_evoked = [-window_length_percentileComputation, 400e-3]
...
# pupil size corresponding to beginning and end of each pupil percentile bin
...
for indWindow, wind_l in enumerate(window_length):
    ...
    trial_window_evoked = [-wind_l, 400e-3]
    ...
    ### number of trials/frequency in each pupil block
    nTrials_perFreq_perPupilBlock = session_info['nTrials_perFreq_perPupilBlock'].copy()
    
    
    ### number of trials to subsample of each frequency in each pupil block
    
    nTrials_subsample_evoked[indWindow] = int(session_info['max_nTrials'])
    
    if nTrials_subsample_evoked[indWindow] < nTrials_thresh:
        print(session_name)
    ...
    session_info = fcn_makeTrials_spont(session_info, wind_l, inter_window_interval)
    ...
    nTrials_subsample_spont[indWindow] = int(np.min(nTrials_in_pupilBlocks))
    
    if nTrials_subsample_spont[indWindow] < nTrials_thresh:
        print(session_name)
        sys.exit('not enough trials')
...
nTrials_subsample = np.min(np.append(nTrials_subsample_evoked, nTrials_subsample_spont)).astype(int)
...
for indWindow, wind_l in enumerate(window_length):
    ...
    t_window, singleTrial_spikeCounts_evoked = fcn_compute_windowed_spikeCounts(session_info, wind_l, window_step)
    ...
    spont_fano = np.ones((nCells, nFreq, n_pupilBlocks, n_subsamples))*np.nan
    evoked_fano = np.ones((nCells, nFreq, n_pupilBlocks, n_subsamples, nWindows))*np.nan
    
    diff_fanofactor = np.ones((nCells, nFreq, n_pupilBlocks, nWindows))*np.nan
    diff_fanofactor_pre = np.ones((nCells, nFreq, n_pupilBlocks, nWindows))*np.nan
    
    spont_trialAvg_spikeCount = np.ones((nCells, n_pupilBlocks))*np.nan
    evoked_trialAvg_spikeCount = np.ones((nCells, nFreq, n_pupilBlocks, nWindows))*np.nan
    pre_trialAvg_spikeCount = np.ones((nCells, nFreq, n_pupilBlocks))*np.nan
    
    # average pupil size of low and high pupil trials
    avg_pupilSize_spontTrials = np.ones((n_pupilBlocks, n_subsamples))*np.nan
    avg_pupilSize_evokedTrials = np.ones((n_pupilBlocks, nFreq, n_subsamples))*np.nan
    ...
    if ( (restOnly == True) and (trialMatch == False) ):
        fName_end = 'trialMatch_windL_restOnly'
            
    else:
        fName_end = 'trialMatch_windL'
        
    
    save_filename = ( (outpath + 'spont_evoked_fanofactor_pupilPercentile_raw_%s_windLength%0.3fs_pupilLag%0.3fs_%s.mat') % (session_name, wind_l, pupilLag, fName_end) )      
