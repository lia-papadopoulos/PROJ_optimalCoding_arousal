...
minEnergy_pathCoords_SaddletoA = np.zeros((saddlePoint_ind_nu2+1, 2))
minEnergy_pathInds_SaddletoA = np.zeros((saddlePoint_ind_nu2+1, 2), dtype='uint32')
...
for rowInd in range(saddlePoint_ind_nu2,-1,-1):
    ...
    minEnergy_pathInds_SaddletoA[count, 0] = minEnergy_ind
    minEnergy_pathCoords_SaddletoA[count, 0] = pop1_rates[minEnergy_ind]

    minEnergy_pathInds_SaddletoA[count, 1] = rowInd                
    minEnergy_pathCoords_SaddletoA[count, 1] = pop2_rates[rowInd]
    
    count += 1
    

# mirror the path for going from saddle to B
minEnergy_pathInds_SaddletoB = np.flip(minEnergy_pathInds_SaddletoA, axis=1)
minEnergy_pathCoords_SaddletoB = np.flip(minEnergy_pathCoords_SaddletoA, axis=1)
...
for pathInd in range(0, np.shape(minEnergy_pathInds_SaddletoA)[0]):
    ...
    xInd = minEnergy_pathInds_SaddletoA[pathInd, 0].astype(int)
    yInd = minEnergy_pathInds_SaddletoA[pathInd, 1].astype(int)
    
    force_minEnergy_path_SaddletoA[pathInd, 0] = F_pop1[yInd, xInd]
    force_minEnergy_path_SaddletoA[pathInd, 1] = F_pop2[yInd, xInd]    

    phiIn_minEnergy_path_SaddletoA[pathInd, 0] = nu1_grid[yInd, xInd]
    phiIn_minEnergy_path_SaddletoA[pathInd, 1] = nu2_grid[yInd, xInd]
...
phiIn_dot_dr_SaddletoA = np.zeros(np.shape(minEnergy_pathInds_SaddletoA)[0]-1)
phiIn_dot_dr_SaddletoB = np.zeros(np.shape(minEnergy_pathInds_SaddletoB)[0]-1)

dr_magnitude_SaddletoA = np.zeros(np.shape(minEnergy_pathInds_SaddletoA)[0]-1)
...
for count in range(0, len(dr_magnitude_SaddletoA)):

    # dr vector along step
    dr = minEnergy_pathCoords_SaddletoA[count + 1, :] - minEnergy_pathCoords_SaddletoA[count, :] 
       
    # F vector along step
    Fstep = force_minEnergy_path_SaddletoA[count,:]
...
# step along path from Saddle to B and store F*dr at each point
for count in range(0, len(dr_magnitude_SaddletoB)):
    
    # dr vector along step
    dr = minEnergy_pathCoords_SaddletoB[count + 1, :] - minEnergy_pathCoords_SaddletoB[count, :] 
    
    # F vector along step
    Fstep = force_minEnergy_path_SaddletoB[count,:]
    ...
    phiIn_step = phiIn_minEnergy_path_SaddletoB[count,:]
...
# F_dot_dr_AtoSaddle = -np.flip(F_project_dr_SaddletoA)
...
path_position_SaddletoB = np.cumsum(np.append(0,dr_magnitude_SaddletoB)) 

# path position
# include Saddle point in A to Saddle, but not in Saddle to B (don't want to double count)
# don't include last data points since we appended 0 at the beginning
# since we are flipping SaddletoA, the [1:] is removing the last data point but keeping Saddle
# since are keeping SaddletoB as is, [1:-1] is removing the saddle point and the last point
path_position = np.append(np.flip(path_position_SaddletoA)[1:], path_position_SaddletoB[1:-1])

# transfer function
transfer_of_r = F_project_dr + path_position
...
for i in range(0,np.shape(minEnergy_pathCoords_SaddletoA)[0]-1):
    
    plt.plot([minEnergy_pathCoords_SaddletoA[i,0],minEnergy_pathCoords_SaddletoA[i+1,0]],\
             [minEnergy_pathCoords_SaddletoA[i,1],minEnergy_pathCoords_SaddletoA[i+1,1]],\
...
plt.figure()
plt.plot(path_position, F_dot_dr, '-o', color='k')
plt.ylabel('F * dr')
...
plt.figure()
plt.plot(path_position, F_project_dr, '-o', color='k')
plt.ylabel('F(r)')
plt.xlabel('position r')
plt.tight_layout()
...
plt.figure()
plt.plot(path_position, U_of_r, '-o', color='k')
...
plt.figure()
plt.plot(path_position, transfer_of_r, '-', color='r')
...
results_dictionary = {'sim_params':                     params, \
                      ...
                      'force_mag':                      force_mag, \
                      'minEnergy_pathCoords_SaddletoA': minEnergy_pathCoords_SaddletoA, \
