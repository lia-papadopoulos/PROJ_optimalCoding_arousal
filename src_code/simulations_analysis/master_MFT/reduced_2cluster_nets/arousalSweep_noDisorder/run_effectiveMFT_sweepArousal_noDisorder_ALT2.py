...
minEnergy_pathInds_AtoSaddle = np.zeros((saddlePoint_ind_nu2+1, 2), dtype='uint32')


# decrease value of nu2 from saddle point to lower right fixed point
# find path of least resistance at each point
for rowInd in range(saddlePoint_ind_nu2,-1,-1):
...
# mirror the path for going from fixed_pointB to saddle
minEnergy_pathInds_BtoSaddle = np.flip(minEnergy_pathInds_AtoSaddle, axis=1)
minEnergy_pathCoords_BtoSaddle = np.flip(minEnergy_pathCoords_AtoSaddle, axis=1)
...
force_minEnergy_path_BtoSaddle = np.zeros((np.shape(minEnergy_pathInds_BtoSaddle)[0], 2))

phiIn_minEnergy_path_AtoSaddle = np.zeros((np.shape(minEnergy_pathInds_AtoSaddle)[0], 2))
phiIn_minEnergy_path_BtoSaddle = np.zeros((np.shape(minEnergy_pathInds_BtoSaddle)[0], 2))
...
# Saddle to B
for pathInd in range(0, np.shape(minEnergy_pathInds_BtoSaddle)[0]):
    
    xInd = minEnergy_pathInds_BtoSaddle[pathInd, 0].astype(int)
    yInd = minEnergy_pathInds_BtoSaddle[pathInd, 1].astype(int)
    
    force_minEnergy_path_BtoSaddle[pathInd, 0] = F_pop1[yInd, xInd]
    force_minEnergy_path_BtoSaddle[pathInd, 1] = F_pop2[yInd, xInd]    
    
    phiIn_minEnergy_path_BtoSaddle[pathInd, 0] = nu1_grid[yInd, xInd]
    phiIn_minEnergy_path_BtoSaddle[pathInd, 1] = nu2_grid[yInd, xInd]  
...
F_project_dr_BtoSaddle = np.zeros(np.shape(minEnergy_pathInds_BtoSaddle)[0]-1)


F_dot_dr_AtoSaddle = np.zeros(np.shape(minEnergy_pathInds_AtoSaddle)[0]-1)
F_dot_dr_BtoSaddle = np.zeros(np.shape(minEnergy_pathInds_BtoSaddle)[0]-1)

phiIn_dot_dr_AtoSaddle = np.zeros(np.shape(minEnergy_pathInds_AtoSaddle)[0]-1)
phiIn_dot_dr_BtoSaddle = np.zeros(np.shape(minEnergy_pathInds_BtoSaddle)[0]-1)

dr_magnitude_AtoSaddle = np.zeros(np.shape(minEnergy_pathInds_AtoSaddle)[0]-1)
dr_magnitude_BtoSaddle = np.zeros(np.shape(minEnergy_pathInds_BtoSaddle)[0]-1)
...
count = 0
for rowInd in range(0, len(dr_magnitude_BtoSaddle)):
    
    # dr vector along step
    dr = minEnergy_pathCoords_BtoSaddle[rowInd+1, :] - minEnergy_pathCoords_BtoSaddle[rowInd, :] 
    
    # F vector along step
    Fstep = force_minEnergy_path_BtoSaddle[rowInd,:]

    # F project dr
    F_project_dr_BtoSaddle[count] = np.dot(Fstep, dr/np.linalg.norm(dr))

    # F dot dr
    F_dot_dr_BtoSaddle[count] = np.dot(Fstep, dr)

    phiIn_step = phiIn_minEnergy_path_BtoSaddle[rowInd,:]
    phiIn_dot_dr_BtoSaddle[count] = np.dot(phiIn_step, dr)
    
    # dr magnitude
    dr_magnitude_BtoSaddle[count] = np.sqrt(np.sum(dr**2))
...
F_project_dr = np.append(F_project_dr_AtoSaddle, -np.flip(F_project_dr_BtoSaddle))

# total force dot dr
F_dot_dr = np.append(F_dot_dr_AtoSaddle, -np.flip(F_dot_dr_BtoSaddle))

# total potential
U_of_r = np.cumsum(-F_dot_dr)[:-1]
U_of_r = U_of_r + np.abs(np.min(U_of_r))

# path distance
path_distance_AtoSaddle = np.cumsum(np.append(0,dr_magnitude_AtoSaddle[:-1]))
path_distance_SaddletoB = np.cumsum(np.flip(dr_magnitude_BtoSaddle)) + path_distance_AtoSaddle[-1]

path_distance_force = np.append(path_distance_AtoSaddle,path_distance_SaddletoB)
path_distance_potential = path_distance_force[:-1]

path_distance_potential = path_distance_potential-path_distance_potential[-1]/2
path_distance_force = path_distance_force-path_distance_force[-1]/2

# transfer function
transfer_of_r = F_project_dr + path_distance_force
...
for i in range(0,np.shape(minEnergy_pathCoords_AtoSaddle)[0]-1):
    ...
    plt.plot([minEnergy_pathCoords_BtoSaddle[i,0],minEnergy_pathCoords_BtoSaddle[i+1,0]],\
             [minEnergy_pathCoords_BtoSaddle[i,1],minEnergy_pathCoords_BtoSaddle[i+1,1]],\
             '-o',color='r',markersize=2)
...
plt.figure()
plt.plot(path_distance_force, F_dot_dr, '-o', color='k')
plt.ylabel('F * dr')
...
plt.figure()
plt.plot(path_distance_force, F_project_dr, '-o', color='k')
plt.ylabel('F(r)')
plt.xlabel('position r')
plt.tight_layout()
...
plt.figure()
plt.plot(path_distance_potential, U_of_r, '-o', color='k')
...
plt.figure()
plt.plot(path_distance_force, transfer_of_r, '-', color='r')
...
results_dictionary = {'sim_params':                     params, \
                      ...
                      'minEnergy_pathCoords_BtoSaddle': minEnergy_pathCoords_BtoSaddle, \
                      'path_distance_force':                  path_distance_force, \
                      'path_distance_potential':                  path_distance_potential, \
                      'F_dot_dr':                         F_dot_dr, \
...
fName_end = ('_effectiveMFT_ALT2_%s.mat' % (sweep_param_str_val))
